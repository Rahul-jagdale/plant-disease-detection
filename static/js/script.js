...
function downloadReport() {
  ...
  doc.text('AGRI SCAN Report', pageWidth / 2, 18, { align: 'center' });
  ...
  doc.setFontSize(8);
  doc.text('Generated by AGRI SCAN - Empowering Farmers Worldwide', pageWidth / 2, 288, { align: 'center' });

  // ── Save ────────────────────────────────────
  var cleanName = (data.disease_name || 'Report').replace(/[^a-zA-Z0-9_]/g, '_');
  var filename = 'AGRI_SCAN_Report_' + cleanName + '.pdf';
  ...
  try {
    ...
    console.error("Blob download failed, falling back to standard jsPDF save", e);
    doc.save(filename);
  }
